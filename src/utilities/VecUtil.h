...
public:
	...
	static constexpr float length_sq(const sf::Vector2<T>& vector)
	{
		return vector.x * vector.x + vector.y * vector.y;
	}
	static constexpr float length_opt(const sf::Vector2<T>& vector)
	{
	...
	static constexpr float length_sq(const sf::Vector2<T>& from, const sf::Vector2<T>& to)
	{
		return length_sq(direction(from, to));
	}
	static constexpr float length_opt(const sf::Vector2<T>& from, const sf::Vector2<T>& to)
	{
	...
	{
		return angle(lhs, rhs, length(lhs), length(rhs));
	}
	...
	{
		return normalize(vector, length(vector), radius);
	}
