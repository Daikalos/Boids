...
struct Config
{
	...
	static float debug_update_freq;
	static int debug_toggle_key;
	...
	static void load()
	{
		...
		if (project_file.good())
		{
			...
			try
			{
				json = nlohmann::json::parse(project_file);
	...
	static void load_var(nlohmann::json& json)
	{
	...
	static void load_static(nlohmann::json& json)
	{
		auto config = json[FILE_NAME];
