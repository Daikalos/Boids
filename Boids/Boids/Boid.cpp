...
void Boid::update(const sf::Window* window, const float& deltaTime, const std::vector<Boid>& boids)
{
	...
	rotation = Vector2::angle(velocity) + util::to_radians(180.0f);
...
std::vector<Boid> Boid::visible_boids(const std::vector<Boid>& boids)
{
	...
	for (const Boid& b : boids)
	{
		...
		{
			sf::Vector2f dir = Vector2::direction(position, b.get_position());
...
sf::Vector2f Boid::seperate(const std::vector<Boid>& boids)
{
	...
	for (const Boid& b : boids)
	{
		float distance = Vector2::distance(b.get_position(), position);
...
sf::Vector2f Boid::cohesion(const std::vector<Boid>& boids)
{
	...
	sf::Vector2f desired = Vector2::direction(coh, position);
